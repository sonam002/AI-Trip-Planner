    ...
    try {
      ...
      const items = snaps.docs.map(d => {
        ...
        if (!data.title) {
          ...
          data.title = `Trip (${tourCount} locations) - ${dateStr}`;
        }
        return { id: d.id, ...data };
      });
      setPlansList(items);
        ...
        <div className="mt-4 mb-8 flex flex-col gap-3">
          ...
          {isCollaborating && sharedPlanId && (
            <div className="text-sm text-gray-600">
              Current Plan: <span className="font-semibold">{planTitle || "Untitled Plan"}</span> | Share id: <span className="font-semibold">{sharedPlanId}</span>
              <button
                onClick={() => {
                  try {
